    ...
    public class RaporBS : BusinessBase
    {
        ...
        private DataTable pKapiTipineGoreSatilanAdet(string il, string ilce, string yil)
        {
            ...
            string sqlText = @"DECLARE @Period AS DATE;
                                ...
                                SELECT
                                    ...
                                    , CAST(ISNULL((SELECT SUM(CAST(ISNULL(ADET,'0') AS INT)) FROM KAPI_FILITRELE WHERE SIPARISTARIH >= DATEADD(MONTH,1,@Period) AND SIPARISTARIH < DATEADD(MONTH,12,@Period) AND KapiTipi=KF.KapiTipi),0) AS VARCHAR) AS 'Yillik'
                                    , '' AS [Yuzde(%)]
        ...
        private DataTable IlSatilanAdetGetir(string il, string yil)
        {
            ...
            string sqlText = @"DECLARE @Period AS DATE;
                                ...
                                SELECT
                                    ...
                                    , CAST(ISNULL((SELECT SUM(CAST(ISNULL(ADET,'0') AS INT)) FROM KAPI_FILITRELE WHERE SIPARISTARIH >= DATEADD(MONTH,1,@Period) AND SIPARISTARIH < DATEADD(MONTH,12,@Period) AND Il=KF.Il),0) AS VARCHAR) AS 'Yillik'
                                    , '' AS [Yuzde(%)]
		                        FROM KAPI_FILITRELE AS KF
		                        GROUP BY Il";
        ...
        private DataTable IlceSatilanAdetGetir(string il, string ilce, string yil)
        {
            ...
            string sqlText = @"DECLARE @Period AS DATE;
                                ...
                                SELECT
		                            ...
		                            , CAST(ISNULL((SELECT SUM(CAST(ISNULL(ADET,'0') AS INT)) FROM KAPI_FILITRELE WHERE SIPARISTARIH >= DATEADD(MONTH,1,@Period) AND SIPARISTARIH < DATEADD(MONTH,12,@Period) AND IlIlce=KF.IlIlce),0) AS VARCHAR) AS 'Yillik'
                                    , '' AS [Yuzde(%)]
                                FROM KAPI_FILITRELE AS KF
		                        GROUP BY Il,IlIlce";
        ...
        private DataTable SemtSatilanAdetGetir(string il, string ilce, string yil)
        {
            ...
            string sqlText = @"DECLARE @Period AS DATE;
                                ...
                                , CAST(ISNULL((SELECT SUM(CAST(ISNULL(ADET,'0') AS INT)) FROM KAPI_FILITRELE WHERE SIPARISTARIH >= DATEADD(MONTH,1,@Period) AND SIPARISTARIH < DATEADD(MONTH,12,@Period) AND Semt=KF.Semt),0) AS VARCHAR) AS 'Yillik' 
                                , '' AS [Yuzde(%)]
		                    FROM KAPI_FILITRELE AS KF

...
namespace ACKSiparisTakip.Web
{
    public class CaptchaImage
    {
        ...
        public Color ElipsForeColor
        {
            get { return this.elipsforeColor; }
            //set { this.foreColor = value; }
        }
        ...
        private Color elipsforeColor = Color.FromArgb(66, 50, 100);
        ...
        // ====================================================================
        public CaptchaImage(string s, int width, int height, string familyName, Color foreColor, Color backgroundColor)
        {
            this.text = s;
            this.SetDimensions(width, height);
            this.SetFamilyName(familyName);
        ...
        private void GenerateImage()
        {
            ...
            {
                fontSize--;
                font = new Font("DaunPenh", fontSize, FontStyle.Bold);
            ...
            Color clrt1 = this.ElipsForeColor;
            Color clrt2 = this.ElipsForeColor;
        ...
        public static string GenerateRandomCode(CaptchaType ctype, int numberOfChars)
        {
            ...
            string alphass = "abcdefghijklmnopqrstuvyz";
            ...
            for (int i = 0; i < numberOfChars; i++)
            {
                ...
                else if (ctype == CaptchaType.Alpha)
                {
                    int k = rnd.Next(1);
                    if (k == 0)
                        s = String.Concat(s, alphanumerics.Substring(rnd.Next(8), 1));
                    else s = String.Concat(s, alphanumerics.Substring(rnd.Next(8), 1));
                }
                else
                {
                    s = String.Concat(s, alphanumerics.Substring(rnd.Next(8), 1));
